...
| Test_Method (선택) | Test_Method, Test Method, Method, 시험방법, 분석방법 |
...
- 필수 컬럼을 찾지 못하면 "다음 필수 컬럼을 인식하지 못했습니다: Moisture, Purity"처럼 정확히 알려주고, 원본 컬럼을 **직접 선택**할 수 있는 칸을 보여줍니다. 모두 연결되기 전에는 `이대로 분석하기` 버튼이 비활성화됩니다. 선택 컬럼인 Test_Method는 찾지 못해도 오류가 아니며, 시험방법 변경 분석만 건너뜁니다.
...
- `test_before_after.py` — 변경 전·후 형식 조합 4가지(Wide/Long), 결합·날짜순 정렬, 전후 평균·변화량·변화율(0으로 나누는 경우 포함), 중복 Lot·날짜 역전 경고, Test_Method 유무 4가지, 기존 단일 분석·4M·AI 요약 유지
...
- 시험방법 컬럼의 이름이 [6.2](#62-coa-컬럼-자동-매핑) 목록에 없으면 시험방법 정보가 없는 것으로 처리됩니다. 시험방법을 분석하려면 목록의 이름(예: `Test_Method`, `시험방법`)을 사용하세요.
...
- **공급사마다 다른 COA 양식을 받습니다** — CSV·Excel, 대표적인 Wide·Long Format과 한글·영문 컬럼명 변형을 자동 인식하고, 인식하지 못한 필수 컬럼은 직접 선택할 수 있습니다.
...
- COA 자동 인식은 대표적인 Wide / Long 형식과 정해진 컬럼명 목록 기준입니다. 기업마다 COA 구조가 다르므로 모든 COA 형식을 자동으로 처리한다고 보장하지 않습니다. 목록에 없는 이름의 필수 컬럼은 화면에서 직접 선택해야 하고, 선택 컬럼인 Test_Method는 목록에 없는 이름이면 인식되지 않아 시험방법 정보가 없는 것으로 처리됩니다.
